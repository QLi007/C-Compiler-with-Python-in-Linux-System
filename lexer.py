...
token_type = {
    ...
    'case':412,
    'default':413,
    'type':301, #void, char, int, float,struct(part3 extra)
    #'void':301,
    #'char':301,
    #'int':301,
    #'float':301,
...
def printf(lst):
    for l in lst:
        print('token value',l.value,'token type',l.type,'token line', l.lineno)
    ...
    while i < len(fp) - 1:
        ...
        #char lit
            ...
            if len(cstr) > char_limit:
                ...
                #add to token list
                add(res,cstr[:char_limit],token_type['str_lit'], lineno)
            else:
                add( res,cstr,token_type['str_lit'], lineno)
                print_log(res,filename)
        ...
        #digit
            ...
            if fp[i] == '0' and (fp[i+1] == 'x' or fp[i+1]=='X'):
                ...
                while fp[i].isdigit() or 'a' <=fp[i] <='f' or 'A'<= fp[i]<='F':
                    istr += fp[i]
                    i += 1
                add(res,istr,token_type['int_lit'], lineno)
                print_log(res,filename)                
            #8
            elif fp[i] == '0':
                i += 1
                ...
                if i < len(fp) and fp[i] == '.' :
                    ...
                    while i < len(fp) and (fp[i].isdigit() or (nume < 1 and  ( fp[i] == 'e' or fp[i] =='E' )) or \
                        ...
                        if fp[i]=='+' or fp[i] =='-':
                            numplus_minus += 1
                        i += 1
                    add(res,istr,token_type['real_lit'], lineno)
                    print_log(res,filename)                    
        ...
        #id
                ...
                while not fp[i].isspace():
                    #if fp[i] == '\n':
                        #lineno += 1
                    if not fp[i].isdigit() and not fp[i].isalpha():
                        break
                    _str += fp[i]
                    i += 1
                #key
...
class Lexer():
    ...
    def getToken(self):
        lst =  get_word(fname,tokenlst)
        for t in lst:
            print(t.value, t.type,t.lineno)
            #self.tokenlst.append(t)
        return lst
    def getIthToken(self, index = 0):
        #if index >= len(tokenlst):
        #    return None
        return self.getToken().get(index)   
...
if __name__ == '__main__':
    ...
    for t in lst:
        print(t.value, t.type,t.lineno)
    print('9th value is ',lst[9].value)
    print(len(lst))
