...
#first use getword get all tokens, include '#', then deal with the tokenlst
# # could followed with define, undef, include, ifdef,ifndef, else, endif
...
def get_word(filename,res):
    ...
    while i < len(fp):
        ...
        if fp[i] == '#': #先record all token in this file， easy to deal with the token after #, since they are in one line
            tr = '#'
            i += 1
...
#depth record the depth of includes files
filedepth = 0
ifdepth = 0 # use endif 
elsecount = 0
ifflag = []
definedepth = 0
fvisited = [] # record the file we lexered, if visited, and met again, then it will report error
definedid = [] # record define id
error_log = []


def part2_lexer(tokenlst, fname):

    #depth record the depth of includes files
    global filedepth 
    global ifdepth # use endif 
    global definedepth 
    global fvisited # record the file we lexered, if visited, and met again, then it will report error
    global definedid  # record define id
    global error_log
    global elsecount
    global ifflag

    curfile = fname
    fvisited.append(fname)

    i = 0 #index
    while i in range(len(tokenlst)):
        if tokenlst[i].value == '#':
            numflag = tokenlst[i].lineno
            i += 1
            while tokenlst[i].lineno == numflag and i < len(tokenlst):
                if tokenlst[i].value == 'include':
                    i += 1
                    if tokenlst[i].type == 305: #str_lit, "filename"
                        if tokenlst[i].value in fvisited:
                            err = 'Preprocessor error in file' + curfile + ' line   ' + tokenlst[i].lineno + ' cycle include files '
                            error_log.append(err)
                            break
                        else:
                            
                            print('..'*filedepth + 'File ' + curfile + ' Line    ' ,str(tokenlst[i].lineno), ' include expansion ')
                            fvisited.append(tokenlst[i].value)
                            curfile = tokenlst[i].value
                            filedepth += 1

                            includetokenlst = []
                            get_word(curfile[1:-1],includetokenlst)
                            part2_lexer(includetokenlst, curfile[1:-1])
                    
                            #back to parent file
                            fvisited.remove(tokenlst[i].value)
                            curfile = fname
                            filedepth -= 1
                            #
                            break #no break, code while stuck here
                    else:
                        err = 'Preprocessor error in file' + curfile + ' line   ' + tokenlst[i].lineno + ' should have string type '
                        error_log.append(err)
                        break
                elif tokenlst[i].value == 'define':
                    i += 1
                    msg = '..'*definedepth + 'File ' + curfile + ' Line    '  + str(tokenlst[i].lineno) +  ' macro expansion '
                    #print(msg)
                    definedepth += 1
                    if tokenlst[i].type == 306:
                        definevalue = tokenlst[i].value
                        if definevalue in definedid:
                            err = 'Preprocessor error in file' + curfile + ' line   ' + tokenlst[i].lineno + ' redefined  ' + definevalue
                            error_log.append(err)
                        else:
                            definedid.append(definevalue)

                        i += 1
                        if tokenlst[i].lineno == numflag:
                            print (msg)
                        while tokenlst[i].lineno == numflag:
                            print('..'*definedepth + 'Macro ' + definevalue + ' Token ' + str(tokenlst[i].type) + ' Text '+ tokenlst[i].value)
                            i += 1
                        definedepth -= 1
                    else:
                        err = 'Preprocessor error in file' + curfile + ' line   ' + tokenlst[i].lineno + ' should have Identifier type '
                        error_log.append(err)
                        break

                elif tokenlst[i].value == 'undef':
                    i += 1
                    definedid.remove(tokenlst[i].value)
                    i += 1 #skip the defined id
                    break
                    
                elif tokenlst[i].value == 'ifdef':
                    ifdepth += 1
                    elsecount += 1
                    i += 1
                    if tokenlst[i].value  not in definedid:
                        ifflag.append(0)
                        i += 1
                        '''
                        endifmark = 1
                        while endifmark > 0:
                            if tokenlst[i].value == '#' and  (tokenlst[i+1].value == 'ifdef' or tokenlst[i+1].value == 'ifndef'):
                                endifmark += 1
                                elsecount += 1
                                i += 2
                            elif tokenlst[i].value == '#' and  tokenlst[i+1].value == 'endif' :
                                endifmark -= 1
                                elsecount -= 1
                                i += 2
                            elif tokenlst[i].value == '#' and tokenlst[i+1].value == 'else' and endifmark <2:
                                i += 2
                                elsecount -= 1
                                ifflag.pop() #rm last elem
                                break
                            elif tokenlst[i].value == '#' and tokenlst[i+1].value == 'else' and endifmark >=2:
                                elsecount -= 1
                                i += 2
                        ...
                        '''
                        while tokenlst[i].value != '#' and tokenlst[i+1].value != 'endif':
                            if tokenlst[i].value == '#' and tokenlst[i+1].value == 'else':
                                i += 2
                                elsecount -= 1
                                ifflag.pop()
                                break
                        ...
                        i += 1
                        #print(tokenlst[i].value, tokenlst[i].lineno)
                        ifflag.append(1)
                        break
                    
                elif tokenlst[i].value == 'ifndef':
                    ifdepth += 1
                    elsecount += 1
                    i += 1
                    #print(tokenlst[i].value, tokenlst[i].lineno)
                    if tokenlst[i].value in definedid:
                        ifflag.append(0)
                        i += 1
                        '''
                        endifmark = 1
                        ...
                        i += 1 # skip the following token
                        ifflag.append(1)
                        break

                elif tokenlst[i].value == 'else':
                    #print(elsecount, tokenlst[i].lineno)
                    
                    if elsecount <= 0:
                        err = 'Preprocessor error in file' + curfile + ' line   ' + str(tokenlst[i].lineno) + ' ,#else without corrrespond #if, ignoring '
                        error_log.append(err)
                        i += 1
                        break
                        ...
                        if  len(ifflag) >0 and ifflag[-1] == 1:
                            while tokenlst[i].value != '#' and tokenlst[i+1].value != 'endif':
                                i += 1
                            break
                        if len(ifflag) >0:
                ...
                elif tokenlst[i].value == 'endif':
                    #print(tokenlst[i].value, tokenlst[i].lineno)
                    
                    
                    if ifdepth <= 0:
                        #ifflag.pop() #rm last elem
                        err = 'Preprocessor error in file' + curfile + ' line   ' + str(tokenlst[i].lineno )+ ' #endif without #if, ignoring '
                        error_log.append(err)
                        i += 1
                        break
                    else:
            ...
            print('..'*filedepth + 'File ' + curfile + ' Line    ' ,str(tokenlst[i].lineno), ' ' + str(tokenlst[i].type) + ' ' + tokenlst[i].value )
            i += 1
            if i >= len(tokenlst):
                return
