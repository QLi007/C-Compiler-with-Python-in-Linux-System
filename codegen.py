    ...
    if index >= len(tokenlst):
        ...
        while index < len(tokenlst):
            ...
            if parse_funcdec():
                ...
                #funcdef
                #funcbody type set during codegen tree
                elif parse_funcbody():
                    #add child
...
# file head
msg = '.class public ' + fname[:-2]
#print(msg)
cout.append(msg)
msg = '.super java/long/Object '
cout.append(msg)
...
def codegen(node):
    ...
    #vardec node already have type
        ...
        if curfunc == '' and curstruct == []:
            for ids in node.children:
                for id in ids.children:
                    if id.children != []:
                        ...
                        codegen(id.children[0]) # id use expr to init
            ...
            if node.type == 'int':
                msg = '.field public static' + ' '+ id.value + ' I'
                cout.append(msg)
            elif node.type == 'float':
                msg = '.field public static' + ' '+ id.value + ' II'
                cout.append(msg)
            else:
                msg = '.field public static' + ' '+ id.value + ' C'
                cout.append(msg)
            ...
            for ids in node.children:
                for id in ids.children:
                    if id.children != []:
                        ...
                        codegen(id.children[0]) # id use expr to init
                        id.type = id.children[0].type
                        msg = '     ;; local 0 ' + id.children[0].value + ' ' + fname
                        cout.append(msg)
        ...
        if node.value in funcs and (node.isprototype == 1 and funcs[node.value][1] == 1):
            ...
            tlen = len(node.children[0].children)
            if tlen == 0:
                msg = '.method public static ' + node.value + ' : ()I'
            elif tlen == 1:
                msg = '.method public static ' + node.value + ' : (I)I'
            elif tlen == 2:
                msg = '.method public static ' + node.value + ' : (II)I' 
            elif tlen == 3:
                msg = '.method public static ' + node.value + ' : (III)I'    
            elif tlen == 4:
                msg = '.method public static ' + node.value + ' : (IV)I'    
            elif tlen == 5:
                msg = '.method public static ' + node.value + ' : (V)I'  
            else:
                msg = '.method public static ' + node.value + ' : (V)I'  
            cout.append(msg)
        ...
        tlen = len(localparam) + len(funcs[curfunc][3])
        msg = '     .code stack ' + str(tlen) + ' locals ' + str(tlen)
        cout.append(msg)
        if len(localparam) > 0:
            for i in range(len(localparam)):
                msg = '         ;; parameter 0 ' + funcs[curfunc][2][0][i] + ' ' + fname
                cout.append(msg)

        cout.append('   .end code')
        cout.append('.end method')
    ...
    #funcbody, just codegen its children
        ...
        if node.children == []:
            if node.isconst == 1: # is isconst ==1, means this is constval
                msg = '         iload_'+ node.value
                cout.append(msg)
            # id()
            if node.isconst == 0: # is isconst ==1, means this is constval
                ...
                if node.value in funcs:
                    ...
                    if node.type == 'int':
                        msg = '         invokestatic Method ' + fname[:-2] +' ' + node.value + '() I' 
                    else:
                        msg = '         invokestatic Method ' + fname[:-2] +' ' + node.value + '() II' 
                    cout.append(msg)
        ...
        if node.children != []:
            if node.children[0] == '?':
                ...
                node.children[2].ancester = 'no'
                codegen(node.children[1]) #expr1
                codegen(node.children[2]) #expr1
            ...
            if funcs[curfunc][2] != [[],[]]:
                for i in range(len(funcs[curfunc][2][0])):
                    ...
                    if checkidin(node.value, funcs[curfunc][2][0][i]):
                        ...
                        #break
                    if idflag == 1:                   
                        msg = '         iload_' + str(i) + '; load from ' + str(funcs[curfunc][2][0][i])
                        cout.append(msg)
        ...
        if idflag == 0:
            ...
            if idflag == 1:                   
                if node.type == 'int':
                    msg = '         getstatic Field ' + fname[:-2] + ' ' + node.value + ' ' + globalvars[node.value]
                else:
                    msg = '         getstatic Field ' + fname[:-2] + ' ' + node.value + ' ' + globalvars[node.value]
                cout.append(msg)
